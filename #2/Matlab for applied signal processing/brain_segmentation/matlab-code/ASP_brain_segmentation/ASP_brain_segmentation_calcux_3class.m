...
function [UX]=ASP_brain_segmentation_calcux_3class(L,Sx,Sy,Sz,label) 
...
% Background values might be changed
TMP=zeros(NL,NC,NS);
TMP(find(NRight==label))=1;
clear NRight
UX=-TMP./Sx;

clear TMP
TMP=zeros(NL,NC,NS);
TMP(find(NLeft==label))=1;
clear NLeft
UX=UX-TMP./Sx;

clear TMP
TMP=zeros(NL,NC,NS);
TMP(find(NTop==label))=1;
clear NTop
UX=UX-TMP./Sy;

clear TMP
TMP=zeros(NL,NC,NS);
TMP(find(NBottom==label))=1;
clear NBottom
UX=UX-TMP./Sy;

clear TMP
TMP=zeros(NL,NC,NS);
TMP(find(NFront==label))=1;
clear NFront
UX=UX-TMP./Sz;
...
clear TMP
TMP=zeros(NL,NC,NS);
TMP(find(NDTopLeft==label))=1;
clear NDTopLeft
UX=UX-TMP./(sqrt(Sx^2+Sy^2));

clear TMP
TMP=zeros(NL,NC,NS);
TMP(find(NDTopRight==label))=1;
clear NDTopRight
UX=UX-TMP./(sqrt(Sx^2+Sy^2));

clear TMP
TMP=zeros(NL,NC,NS);
TMP(find(NDBottomLeft==label))=1;
clear NDBottomLeft
UX=UX-TMP./(sqrt(Sx^2+Sy^2));

clear TMP
TMP=zeros(NL,NC,NS);
TMP(find(NDBottomRight==label))=1;
clear NDBottomRight
UX=UX-TMP./(sqrt(Sx^2+Sy^2));
clear TMP
