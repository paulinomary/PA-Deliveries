...
hamburger= imread("hamburger.png");
%image(picanha);
figure, imshow(hamburger), title('Original Image')

hamburgerYCbCr = rgb2ycbcr(hamburger);
...
subplot(2,3,4);
imshow(hamburgerYCbCr(:,:,1));
title('Componente Y (YCbCr)');

subplot(2,3,5);
imshow(hamburgerYCbCr(:,:,2));
title('Componente Cb (YCbCr)');

subplot(2,3,6);
imshow(hamburgerYCbCr(:,:,3));
